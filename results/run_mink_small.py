...
# Set to a positive integer to run only the first N sorted small maps. For
...
# The paper limits R-ILP to one hour on small-map instances. For a reduced
...
def list_maps(folder: Path, limit: int | None = None) -> list[Path]:
    if not folder.is_dir():
        raise FileNotFoundError(f"Small-map directory not found: {folder}")
...
def main() -> None:
    ...
    map_dir = data_dir / "small_maps_6.9x6.9_tiling"
    tiling_file = data_dir / "tilings" / "6.9x6.9_tiling.csv"
    output_file = (
        repo_root / "results" / "min_telescope" / "mink_small.csv"
    )

    require_file(executable, "ts_mink executable")
    require_file(tiling_file, "6.9x6.9 small-map tiling")
    maps = list_maps(map_dir, MAP_LIMIT)
    prepare_output(output_file)

    for index, map_file in enumerate(maps, start=1):
        print(f"\nSmall-map case {index}/{len(maps)}: {map_file.name}")
        run_case(executable, map_file, tiling_file, output_file)

    print(f"\nCompleted {len(maps)} small-map runs.")
