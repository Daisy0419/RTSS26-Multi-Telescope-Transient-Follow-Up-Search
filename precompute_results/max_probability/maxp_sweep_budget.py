...
def plot_detprob_vs_budget(df: pd.DataFrame,
                           dataset: str,
                           n_path: int,
                           outdir: Path) -> plt.Figure:
    """Detection probability vs Budget for one map at fixed nPath.
    ...
    sub = df[(df["Dataset"] == dataset) & (df["nPath"] == n_path)].copy()
    if sub.empty:
        raise ValueError(f"No rows for dataset={dataset}, nPath={n_path}")

    tiles = int(sub["mapTiles"].iloc[0])
    tag = short_dataset(dataset)
    event = short_dataset(dataset, tiles)
    out = outdir / f"{tag}_detprob_vs_budget.png"
    ...
    # ax.set_title(fr"{event}, $n_{{\mathrm{{path}}}}={n_path}$")
...
def plot_runtime_vs_budget(df: pd.DataFrame,
                           ...
                           n_path: int,
                           outdir: Path) -> plt.Figure:
    """Planning time vs Budget for one map at fixed nPath.
...
if __name__ == "__main__":
    csv_path = Path("maxp_large_map_budget.csv")
    csv_path = Path("maxp_very_large_map_budget.csv")
    outdir = Path("figs")

    fixed_npath = 4

    map = "GW191113_071753_952.txt"
    # map = "GW191113_071753_5957.txt"
    map = "GW200105_162426_11678.txt"
    # map = "GW230529_181500_29327.txt"
    ...
    for dataset in maps:
        figs.append(plot_detprob_vs_budget(df, dataset, fixed_npath, outdir))
        figs.append(plot_runtime_vs_budget(df, dataset, fixed_npath, outdir))
